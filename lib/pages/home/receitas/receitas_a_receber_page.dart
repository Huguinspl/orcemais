...
import 'detalhes_receita_a_receber_page.dart';

/// Página de Receitas a Receber (transações futuras)
/// Mesma estrutura da ReceitasPage, mas filtrando apenas isFutura = true
class ReceitasAReceberPage extends StatefulWidget {
  const ReceitasAReceberPage({super.key});

  @override
  State<ReceitasAReceberPage> createState() => _ReceitasAReceberPageState();
}

class _ReceitasAReceberPageState extends State<ReceitasAReceberPage> {
  ...
  Future<void> _abrirFormulario({Transacao? transacao}) async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => NovaReceitaPage(transacao: transacao, isFutura: true),
      ),
    );
  }

  // Ação para visualizar detalhes da receita
  ...
  // Ação para alterar status da receita (igual aos agendamentos)
  Future<void> _marcarComoRecebida(Transacao transacao) async {
    ...
    final statusConfig = {
      ...
      'Recebido': {
        ...
        'label': 'Recebido',
        'subtitle': 'Marca como recebido e registra receita',
      },
      'Reagendar': {
        'cor': Colors.blue,
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Receita recebida e registrada (${currencyFormat.format(transacao.valor)})',
                  ),
                ),
              ],
            ),
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Receita reagendada para ${dateFormat.format(novaData)}',
                  ),
                ),
              ],
            ),
  ...
  // Ação para editar a receita a receber
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  'Deseja realmente excluir esta receita a receber?',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              Expanded(
                child: Consumer<TransacoesProvider>(
                  builder: (context, provider, child) {
                    ...
                    final listaFiltrada =
                        provider.receitasAReceber.where((receita) {
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        tooltip: 'Nova receita a receber',
        icon: const Icon(Icons.add),
        label: const Text('Nova A Receber'),
        backgroundColor: Colors.teal.shade600,
      ),
    );
  }

  // ✅ Card de receita a receber moderno
